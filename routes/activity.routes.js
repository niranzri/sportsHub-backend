...
// PUT one
router.put("/:activityId", isAuthenticated, async (req, res) => {
  // const { userId } = req.tokenPayload
  const payload = req.body;
  const { activityId } = req.params;
  try {
    const activityToUpdate = await Activity.findById(activityId);
    //if (activityToUpdate.companyId == userId) {
...
// DELETE one
router.delete("/:activityId", isAuthenticated, async (req, res) => {
  // const { userId } = req.tokenPayload
  const { activityId } = req.params;
  try {
    const activityToDelete = await Activity.findById(activityId);

    //if (activityToDelete.companyId == userId) {
